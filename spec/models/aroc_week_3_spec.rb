...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.joins(:order_items => :item)
                .distinct
                .where("order_items.item_id = '#{@item_8.id}'")
                .order('users.name')
                .pluck('users.name')
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order = Order.joins(:order_items => :item)
                                        .distinct
                                        .where("orders.user_id = '#{@user_3.id}' AND orders.amount = '649'")
                                        .pluck('items.name')
    # ------------------------------------------------------------
  ...
  it '20. returns the average amount for all orders for one user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    average = Order.where("orders.user_id = '#{@user_3.id}'")
                   .average(:amount)
    # ------------------------------------------------------------
